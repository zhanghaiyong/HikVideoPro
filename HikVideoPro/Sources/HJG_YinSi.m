...
{
    if (!_webV) {
        UIWebView * theView = [[UIWebView alloc] initWithFrame:CGRectMake(10, kStatusBarHeight, [UIScreen mainScreen].bounds.size.width - 20, [UIScreen mainScreen].bounds.size.height - 80 - kStatusBarHeight - (isIPhoneX ? 34 : 0))];
        theView.backgroundColor = [UIColor whiteColor];
        NSString* path = [[NSBundle mainBundle] pathForResource:@"App隐私政策" ofType:@"html"];
        NSURL* url = [NSURL fileURLWithPath:path];
        NSURLRequest* request = [NSURLRequest requestWithURL:url];
        [theView loadRequest:request];

        UserDefaultSetObjectForKey(@"1", isFirstYinSi);
...
- (instancetype)init{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidFinishLaunchingNotification object:nil queue:nil usingBlock:^(NSNotification * _Nonnull note) {
            if ([UserDefaultObjectForKey(isFirstYinSi) intValue] == 1) {}else{
